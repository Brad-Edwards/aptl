...
- The static gate is a composition of existing authorities: RAES parser,
  semantic validation, import lock verification, runtime compilation,
  canonical RAES backend manifest/conformance, APTL parity inventory, APTL
  inventory ledgers, and APTL backend realization tests.
- `scenarios/techvault.sdl.yaml` is validated as RAES SDL through
  `raes.parse_sdl_file()` and `raes_processor.compiler.compile_runtime_model()`.
...
- Backend manifest completion evidence must use the canonical RAES
  ...
  `raes_contracts.backend_profiles.load_backend_profile()`, and
  `raes conformance backend --profile provisioning-only`. The provisional
  APTL-local manifest shape in `src/aptl/backends/raes_manifest.py` may remain
...
- Static parity checks are evidence gates, not runtime owners. They may read
  `docs/raes/parity-inventory.yaml`, mapping ledgers, compiled RAES runtime
...
- Phase A may expose the gate as advisory, but advisory still means visible,
  ...
  RAES contract assets, or unavailable conformance CLI wiring as waived.

## Cross-Cutting Concerns To Reuse

- RAES SDL and module authorities: `parse_sdl_file`, `SemanticValidator`,
  `compile_runtime_model`, `resolve_lock_records`, `load_lockfile`, and the
  `raes sdl resolve` / `verify-imports` CLI.
- RAES backend contract authorities: `BackendManifest`, `ProvisionerCapabilities`,
  ...
  under `contracts/profiles/backend/`, and the `raes conformance backend`
  fixture corpus under `contracts/fixtures/`.
- APTL RAES adapter seams: `RuntimeManager`, `RuntimeTarget`,
  `src/aptl/backends/raes.py`, `src/aptl/backends/raes_realization.py`,
  `src/aptl/backends/raes_realization_model.py`, `src/aptl/backends/raes_profiles.py`,
  and `tests/test_raes_backend.py`.
- APTL parity and inventory checks: `docs/raes/parity-inventory.yaml`,
  `docs/raes/parity-inventory.md`, `tests/test_parity_inventory.py`,
...
- **RAES SDL shape:** closed-world structural validation and semantic
  cross-reference validation are RAES-owned. APTL diagnostics may name the
  failing file, RAES code, row id, or resource address, but not dump the full
  SDL object or raw exception payload.
- **Import trust and dependency shape:** RAES module resolution owns path
  confinement, trust policy, and lockfile comparison. APTL must not fetch,
  expand, or execute imports through a custom helper.
- **Backend manifest shape:** the manifest must be a real RAES
...
- **Config shape:** durable APTL knobs stay in strict `AptlConfig`; do not add
  RAES-specific keys or pass-through dictionaries to `aptl.json` for the gate.
...
- **Persistence:** generated validation reports are analysis artifacts, not
  ...
  only for run evidence; do not persist raw RAES/APTL objects that may contain
  secret-shaped payloads.

## Extensibility Seam

The seam is the RAES runtime resource kind plus backend capability profile.
The gate should be parameterized by scenario path, backend profile, RAES
...
- Treating `src/aptl/backends/raes_manifest.py` as the final manifest contract.
  Its current dataclasses explicitly describe a subset consumed by installed
  planner/runtime code; #322 must replace or wrap that with the canonical RAES
  manifest payload.
...
- Letting a missing RAES contract corpus degrade to a warning. A local smoke
...
- Treating evidence bundles, checksums, screenshots, logs, or mapping ledgers
  as substitutes for RAES SDL encoding when RAES can express the fact.
...
- Do not file RAES issues as waivers. A linked RAES expressivity issue blocks
