...
# SCN-010 / issue #343 — wazuh-sidecar-db steady-state asset inventory capture.
#
# The wazuh-sidecar-db asset is a *log-forwarding agent sidecar*: a custom
# debian:12-slim image (aptl-wazuh-sidecar:local) running wazuh-agent 4.12.0.
# It has no inbound listener — it dials OUT to the Wazuh manager (1514/1515),
# reads the db container's PostgreSQL log via a read-only bind of the shared
# aptl_db_data volume at /logs, and ships /logs/pg_log/postgresql.log. Capture
# reflects that archetype (no listening-service probe surface; the manager
# connection and the read-only log source are recorded explicitly).
#
...
ASSET_DIR="$ROOT/docs/aces/inventory/wazuh-sidecar-db"
OUT="$ASSET_DIR/evidence"
CONTAINER="${CONTAINER:-aptl-wazuh-sidecar-db}"
...
# The wazuh-sidecar-db compose block carries only plain env (WAZUH_MANAGER,
...
# wazuh apt source + signing key, and os-release. /logs (db PGDATA) is excluded
# (db asset content). client.keys and .ssh are classified secret and excluded
# from checksums below.
...
docker exec "$CONTAINER" sh -lc '
  ...
  ls -la /logs/pg_log/postgresql.log 2>&1 || true
  printf "%s\n" --internal-options--
...
if docker inspect "$MANAGER_CONTAINER" >/dev/null 2>&1; then
  docker exec "$MANAGER_CONTAINER" sh -lc '
    set +e
    printf "%s\n" --agent-list--
    /var/ossec/bin/agent_control -l 2>&1 | grep -iE "aptl-db-agent|Total|Wazuh"
    printf "%s\n" --agent-detail--
...
if docker inspect aptl-kali >/dev/null 2>&1; then
  docker exec aptl-kali sh -lc '
    set +e
    printf "%s\n" --dns--
    getent hosts wazuh-sidecar-db aptl-wazuh-sidecar-db 172.20.0.35 2>&1
    printf "%s\n" --route-to-security-net--
    ip route get 172.20.0.35 2>&1
    printf "%s\n" --tcp-probe--
    timeout 8 sh -c "nc -vz -w 3 172.20.0.35 1514 2>&1; nc -vz -w 3 172.20.0.35 1515 2>&1" 2>&1
    printf "%s\n" --ping--
...
write_osquery_json "$OUT/osquery-processes.json" processes \
  ...
  "containerized osquery sharing aptl-wazuh-sidecar-db PID namespace" container
...
write_osquery_json "$OUT/osquery-listening-ports.json" listening_ports \
  ...
  "containerized osquery sharing aptl-wazuh-sidecar-db network namespace" container

write_osquery_json "$OUT/osquery-docker-containers.json" docker_containers \
  'select id, name, image, image_id, command, created, state, status, pid, path, config_entrypoint, started_at, finished_at, privileged, security_options, readonly_rootfs, cgroup_namespace, ipc_namespace, mnt_namespace, net_namespace, pid_namespace, user_namespace, uts_namespace from docker_containers where name="/aptl-wazuh-sidecar-db";' \
...
(
  cd "$ROOT"
  find docs/aces/inventory/wazuh-sidecar-db/evidence -maxdepth 1 -type f \
