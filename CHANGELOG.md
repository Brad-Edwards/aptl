...
- ### Security
  ...
  now narrowed to Wazuh inter-component traffic only. (#258)
- ### Security

  - Privileged capture writer: move all Kali-side capture writes (PTY transcript, per-session pcap, auditd, process accounting) into a dedicated `aptl-kali-capture` sidecar container that solely owns the `kali_captures` volume. The volume is no longer mounted in the Kali workload at all, so a sudo-capable agent can no longer read, list, delete, or modify any session's capture evidence — including other sessions' (ADR-041, issue #305). (#305)
- ### Security
...
- ### Security
  ...
  - `APTL_API_TOKEN` is required at startup (Docker Compose enforces this with
    ...
    `python3 -c 'import secrets; print(secrets.token_hex(32))'`. (#415)
- ### Security
  ...
  is tracked as a separate follow-up. (#416)
- ### Security
...
- ### Security
  ...
  failing to connect. `workstation` is now a registered SSH endpoint. (#418)
- ### Security
  ...
  via CSRF through the operator's browser. (#521)
- ### Security

  - Harden `LocalRunStore` core archive methods against `run_id` and `relative_path` traversal so run evidence cannot be written outside the intended run archive tree; reject unsafe `--run-id` values at `aptl lab validate-live` ingress (issue #522). (#522)
- ### Security

  - OTel Collector (4317/4318) and Tempo (3200) host ports now bind 127.0.0.1 (loopback) instead of all interfaces, per ADR-034/ADR-039. (#523)
- **Scoped MCP API client auth to configured service origins.** The generic
  ...
  URLs or SSRF to attacker-controlled hosts. (#524)
- **Wazuh Indexer credentials no longer reach `curl` via the command line.** The startup readiness probe and the alert collector now send HTTP Basic auth through a 0600 header file (`Authorization: Basic …`) instead of `-u user:pass`, so the indexer password is no longer observable via `ps` or `/proc/<pid>/cmdline` (ADR-029). (#623)
...
- ### Added
  ...
  - Hypothesis property-based regression tests for the credentials, env,
    ...
    #183 — fail as a test rather than as a hung lab start. (#213)
- ### Added
...
- ### Added
  ...
  - **ACES backend promotion to `orchestration-evaluation` (SCN-010 / #312).**
    ...
    RTE-001 is the follow-on #514. (#312)
- ### Added
...
- **Added a static ACES validation gate for the TechVault scenario (SCN-010E).**
  ...
  blocking at the SCN-010 cutover. (#322)
- **Added a live ACES validation gate for the TechVault scenario (SCN-010F).**
  ...
  never part of fast CI. (#323)
- Added the SCN-010 TechVault composition inventory bundle, participant agent framing, parity mapping, and regression coverage. (#329)
- **Captured the TechVault webapp ACES inventory.** Added the webapp steady-state evidence bundle, mapping ledger, local preflight artifact, TechVault ACES SDL webapp encoding, parity inventory rows, and structural tests for SCN-010 issue #330. The SDL now consumes ACES #358 runtime expressivity for the webapp process set, runtime environment, capability policy, restart/resource policy, full captured package inventory, and full captured Trivy finding inventory. It also consumes ACES #363, #364, #365, #366, #367, and #368 for catalogued runtime filesystem metadata/checksums, container image build provenance, full local identity database facts, Docker network realization metadata, Flask HTTP route/API/UI inventory, full mount facts, container host/security config, and health observations, leaving no known ACES expressivity blocker for the catalogued webapp steady-state inventory facts. Full root filesystem cataloguing is deferred as capture scope, not blocked by ACES expressivity. (#330)
- **Captured the TechVault db ACES inventory.** Added the db steady-state evidence bundle, mapping ledger, TechVault ACES SDL db encoding, parity inventory rows, and structural tests for SCN-010 issue #331. The SDL now records the upstream postgres image identity, init-script provenance, runtime configuration, data-volume filesystem inventory, local identity database, package/CVE state, PostgreSQL logical-state observations, and Wazuh log-forwarding relationship with explicit snapshot boundaries. (#331)
- ### Added

  - Added the TechVault AD container ACES inventory bundle, SDL encoding, parity
    row, and regression coverage for SCN-010 issue #332. (#332)
- Added the SCN-010 mailserver ACES steady-state inventory bundle, evidence
  capture tooling, SDL encoding, parity inventory row, and validation coverage. (#335)
- Added the SCN-010 DNS ACES steady-state inventory bundle, typed DNS runtime
  SDL encoding, parity inventory rows, and validation coverage. (#336)
- **Captured the TechVault victim ACES inventory.** Added the victim evidence bundle, mapping ledger, TechVault SDL encoding, parity inventory row, and regression coverage for SCN-010 issue #337. (#337)
...
- Added the SCN-010 Wazuh manager ACES steady-state inventory bundle, evidence
  ...
  rule/correlation-rule and decoder set. (#340)
- Added the SCN-010 Wazuh indexer ACES steady-state inventory bundle, evidence
  ...
  inventory issue stays open until those ACES surfaces land. (#341)
...
- Added the SCN-010 completion-grade misp-redis ACES asset inventory: a
  ...
  MISP-to-Redis relationship and validation coverage. (#348)
- - Added the ACES backend asset inventory methodology, assurance report,
    ...
    `shuffle-backend` evidence bundle for the TechVault methodology spike. (#353)
...
- Added the SCN-010 completion-grade shuffle-backend ACES asset inventory: a
  ...
  and shuffle-opensearch datastore relationship, with validation coverage. (#360)
- **Added and resolved the TechVault SDL classification audit.** Shuffle backend and Wazuh manager now carry the audited node-local SDL state, switch isolation metadata matches Compose, and stale parity rows are updated with regression coverage. (#369)
- Added an installer that links the ACES asset inventory capture skill into
  Claude/Codex user-level skill locations from a local ACES checkout. (#371)
- Documentation overhaul: Vale prose-lint gate (pre-commit + CI, ADR-038), mkdocs nav reconciled to the docs tree with `mkdocs build --strict` enforced in CI, docs published to GitHub Pages on main, and an accuracy sweep fixing stale access instructions (victim/kali host SSH ports, hardcoded credentials, removed deploy workflow, MCP server inventory). (#407)
- The APTL ACES backend now realizes the declared node health expectation (`runtime.health.status`) from the compiled ACES plan and enforces it during live conformance validation: a node that declares `healthy` whose container is not actually healthy is now a hard readiness failure instead of being silently tolerated. (#422)
- ACES-aligned run reproducibility record written to the run archive on lab start (and `aptl runs list` now shows normal lab runs). (#423)
- ### Added
  ...
  **Ephemeral environments with clean-state guarantees (RNG-001).** A new
  ...
  gate now consumes for its pre-snapshot clean boot. (#424)
- ### Added
...
- ### Added
  ...
  - **RTE-001 workflow execution state for the ACES orchestrator (#514).**
    ...
    and history artifacts through `LocalRunStore` when configured. (#514)
...
- ### Added
  ...
  - Recorded live-boot proof for the curated TechVault ACES startup variants
    ...
    *reduced* Compose surface — not the full TechVault range. A new
    ...
    separate full-surface proof. (#535)
- **Added the APTL web GUI design specification.** The new spec records the v1 product scope, route map, page interaction design, component inventory, API contract gaps, security UX, and implementation checklist for the shipped web GUI surface. (#540)
- **Added APTL's ACES participant runtime.** The runtime target now exposes participant episode control, promotes the manifest to `full-remote-control-plane`, and records a TechVault Kali-to-victim live action proof through the standard ACES control plane. (#554)
- ### Added
  ...
  - Web GUI component kit and documented Tailwind v4 design tokens: reusable
    ...
    `docs/specs/web-component-kit.md`. (#563)
- ### Added
  ...
  - Web Lab Home (`/`) now leads with a readiness-first status headline, adds an
    ...
    card summaries so the Lab Home scenario entry points render. (#564) (#564)
- ### Added
  ...
  - Web scenario workbench (`/scenarios/[id]`) now renders a notebook-style,
    ...
    and SIEM query blocks stay display-only (execution is a later slice). (#565) (#565)
- ### Added
  ...
  - Narrow, accessible terminal rejection reasons on the focused terminal
    ...
    exposed, and terminal input/output is still never persisted. (#566)
- Added the ACES paper `paper-agent-loop` scenario, typed deployment realization
  ...
  snapshot surfaces. (#573)
- **Added ACES image realization.** Scenario node `source` metadata can now drive trusted pull/build image operations through the deployment backend before lab startup. (#574)
- Realize SDL-declared networks, IPAM, internal egress policy, and static node
  attachments through typed deployment backend inputs. (#575)
...
- **Lab start now returns a structured partial-readiness classification (ADR-030).**
  ...
  (optional, additive, backward compatible). (#202)
...
- **Snapshot endpoint maps moved behind a single registry.** Internal
  ...
  change; adding a new endpoint is now one registry edit. See ADR-036. (#261)
- ### Changed
...
- **Per-function complexity gate now covers the TypeScript surfaces, with zero
  ...
  *Pre-commit hooks* CI job and locally, failing any new function over the
  ...
  `src/`, `mcp/*/src/`, and `web/src/` is now gated at 15. See ADR-010. (#286)
- Route `aptl lab start` and `/api/lab/start` container startup through the ACES SDL parser, runtime planner, and APTL provisioning target before invoking the existing deployment backend. (#321)
...
- **Generalized the ACES provisioning backend realization path.** APTL now
  ...
  explicit diagnostics instead of relying on scenario metadata. (#324)
...
- Rehomed ACES asset inventory methodology authority to the ACES docs while
  ...
  tooling as downstream implementation artifacts. (#374)
- ### Changed
  ...
  - Reconciled `scenarios/techvault.sdl.yaml` and the per-container mapping
    ...
    references on every ledger entry are preserved verbatim. (#388)
- ### Changed
  ...
  **Made the target container images and their ACES inventories reproducible.**
  ...
  value. (#417)
- Enforce CI strictness on protected branches: SonarCloud waits for the quality gate and fails on new issues, branch-protection baseline records strict required checks for `main` and `dev`, and the Docs job joins the required merge gate. (#502)
- Retired the APTL-local SDL parser/runtime surfaces after the ACES cutover, archived legacy scenario YAML as reference-only material, and updated docs/tests to use the ACES catalog and validation path. (#530)
- ACES-driven TechVault startup now computes transitive Compose dependency closure for selected subset nodes before starting the backend, adding required support profiles automatically and failing closed with ACES diagnostics when required dependencies are missing, ambiguous, or disabled. (#532)
- ACES realization now validates that the selected Compose profile set forms a valid project. If an activated service depends on a service the profile selection excludes (for example, the enterprise tier without the Wazuh core its workstation reports to), `interpret_provisioning_plan` raises an `aptl.provisioner.compose-project-invalid` diagnostic at realization time instead of failing later with a raw `docker compose` "undefined service" error. (#534)
- **`aptl web serve` now ships the GUI and API from a single loopback origin (FastAPI BFF) with a real session credential.**
  ...
  `HttpOnly`/`Secure`/`SameSite=Strict` cookie plus a port-scoped header token the SPA keeps
  ...
  preserved. `aptl web serve` is always single-worker (the in-process session
  secrets, one-time launch token, and terminal-ticket store are not shared across
  workers), so the unsupported `--workers` flag has been removed. If no GUI build
  is present, `aptl web serve` fails fast unless `--api-only` is passed. The split `aptl-web-api` + `aptl-web-ui` Compose profile
...
- ### Fixed
  ...
  - `aptl.core.config.load_config` now raises `ValueError` (not
    ...
    `(FileNotFoundError, ValueError)` contract. (#213)
- `aptl lab start` now seeds MISP Suricata rule baselines into `.aptl/suricata/rules/misp/` and mounts that generated directory for Suricata and `misp-suricata-sync`, so the sync service no longer rewrites checked-in `config/suricata/rules/misp/` files during lab startup. (#287)
- ### Fixed
...
- Fixed the TechVault fileshare scenario's planted SSH deploy key never being
  ...
  now-present `deploy_key` / `deploy_key.pub` with their stat and sha256. (#341)
- ### Fixed
  ...
  retries on the next interval. (#386)
...
- **`aptl web serve` now issues the session cookie's `Secure` flag based on the connection scheme, so the GUI works over HTTPS fronts and encrypted remote transports instead of only loopback.**
  ...
  `APTL_ALLOWED_HOSTS`, and `--public-origin`. (#582)
- Fixed ACES backend conformance truth-up by rejecting provisioning runs whose
  ...
  realization provenance for honored concerns. (#599)
- Evaluator progress and score surfaces now advance from observed ACES runtime state instead of remaining pending-only placeholders. (#606)
- `aptl lab start` now creates or repairs `.env` automatically with runnable lab credentials, prints a post-start access summary, and adds `aptl lab info` to reprint credential locations later. (#620)
- Added a direct `aptl lab stop` / `aptl lab stop -v` recovery hint when lab startup fails on stale APTL network labels. (#621)
- **Fixed `aptl lab status` for non-`aptl` checkout directories.** Text status now queries the configured Compose project instead of deriving a different project name from the working directory. (#622)
- **Detected stale Wazuh Indexer credentials during `aptl lab start`.** When the indexer's listener is up but rejects the configured `.env` credentials (HTTP 401) — typically because a reused `wazuh-indexer-data` volume still holds the previous admin password — startup now emits a targeted diagnostic pointing at `aptl lab stop -v` recovery instead of a generic "did not become ready" timeout message. (#623)
- Show live `aptl lab start` phase and readiness progress during long startup work. (#624)
- **Improved lab-start network preflight.** `aptl lab start` now reports the exact Docker network and subnet that blocks realized network creation before Compose startup. (#630)
- ### Fixed
...
- **Control-plane secret-handling policy with run-archive boundary redaction and Python/TypeScript command-redaction parity.** APTL now classifies secret-like values into two classes — designed-vulnerable target data (lab user passwords, captured CTF flags, dumps produced by intentional compromise) and control-plane/operator secrets (real API keys, service tokens, bearer tokens, cookies, JWTs, private SSH/TLS keys, `.env` runtime secrets, generated service-config secrets, MCP client env secrets, replayable session identifiers). The invariant: **control-plane/operator secrets are redacted before they cross a serialization, observability, archive, CLI, API-response, or log boundary.** File permissions, ignored paths, and archive locations are defense in depth only. The full policy plus a per-path artifact-boundary audit table (covering `RangeSnapshot.to_dict()`, MCP `traceToolCall()`, `LocalRunStore` `write_json`/`write_jsonl`/`append_jsonl`, `core/collectors.py`, `core/exporter.py`, `cli/runs.py`, `aptl config show --json`, `aptl lab continuity-audit --json` / `continuity-events.jsonl`, and `.aptl/` state) is recorded in [ADR-029](docs/adrs/adr-029-control-plane-secret-handling.md). `LocalRunStore.write_json` / `write_jsonl` / `append_jsonl` now run `redact()` at the persistence boundary so run-archive contents are control-plane-secret-safe by construction (the snapshot DTO and OTel-span boundaries from #185 already redact); `write_file` (opaque bytes) and `copy_file` (arbitrary files) remain pass-through by design — callers must not route control-plane secrets through them, and the runstore's docstring + tests pin that boundary. The Python redaction helper (`src/aptl/utils/redaction.py`) is brought to parity with the TypeScript helper (`mcp/aptl-mcp-common/src/redaction.ts`): a quote- and escape-aware top-level shell-segment splitter (handles `&&`/`||`/`;`/`|`/single `&`, redirections `2>&1`/`&>`/`<&3`, single/double quotes, backslash escapes); per-segment tool-family detectors (credential `-p` tools — hydra/medusa/patator/crowbar/sshpass/wfuzz/kerbrute/crackmapexec/cme/nxc/evil-winrm/bloodhound-python/bloodhound.py/mysql/mariadb/mysqladmin/redis-cli/impacket-* / psexec.py / smbexec.py / wmiexec.py / secretsdump.py / getuserspns.py / getnpusers.py / ntlmrelayx.py; `-H` NTLM-hash tools — crackmapexec/cme/nxc/impacket-*/evil-winrm; `-w` LDAP-bind-password tools — ldapadd/ldapcompare/ldapdelete/ldapmodify/ldappasswd/ldapsearch/ldapwhoami; impacket-family tools); short `-p` password handling (numeric values stay visible *unless* the segment names a credential tool — `nmap -p 22 host && hydra -p 12345 host ssh` keeps the port and masks the password — and quote-form-/equals-form-/attached-form-aware: `-p value`, `-p=value`, `-p<value>`, escape-aware unquoted `correct\ horse`, double- and single-quoted multiword values); `-H <hash>` redaction scoped to hash-using segments so `curl -H 'X-Foo: bar'` is left alone; `-w <password>` redaction scoped to LDAP segments so `hydra -w 5` (wait/wordlist) is left alone; `--user`/`-u`/`-U` Basic-auth + Samba `user%password` masking with URL-target carve-out so `sqlmap -u https://target.example/login` and `gobuster dir -u https://target.example:8443/admin` survive; impacket positional `user:password@host` (and `domain/user:pass@host`) masking with quoted-password support for embedded `:` `@` and whitespace, scoped to impacket segments so an unrelated `user:token@host` elsewhere is left alone; quoted standalone option-token normalization (`'-p'` → `-p`, `"-u"` → `-u`) so a quoted flag still triggers per-flag redaction. Closes #281 (mirror MCP-side command-flag redaction patterns to Python helper). Tests: `tests/test_redaction.py::TestCommandFlagRedaction` mirrors the relevant cases from `mcp/aptl-mcp-common/tests/redaction.test.ts` (positive masks, port/header/wordlist negatives, wrapper-prefix detection, per-segment selectivity, quoted/escaped/attached forms, idempotency); `tests/test_runstore.py::TestLocalRunStoreRedaction` pins the persistence boundary including the documented `write_file`/`copy_file` pass-through; `tests/test_exporter.py::TestExportRedactionChain` pins the runstore→export chain so a control-plane-shaped secret written through `LocalRunStore.write_json`/`write_jsonl` arrives in the exported tar.gz already redacted. Implements [#201](https://github.com/Brad-Edwards/aptl/issues/201); architectural rationale in [ADR-029](docs/adrs/adr-029-control-plane-secret-handling.md).
- **Red-team structured logging — two-channel capture for SIEM correlation and research analysis.** AI-agent-executed Kali commands now feed two parallel sinks: (1) an OCSF-shaped SIEM event stream on `[OCSF]`-prefixed stderr JSONL with `class_uid` / `category_uid` / `activity_id` / `type_uid` / `metadata.product` / `severity_id` / `attacks[]` / `dst_endpoint` / `actor.user` / `http_request.url` / `connection_info.protocol_name` / `file.path` / `status_id` / redacted `process.cmd_line`, and (2) a raw research-grade capture written as JSONL records to `<APTL_STATE_DIR>/red-tool-calls.jsonl` (override via `APTL_RED_CAPTURE_PATH`) covering every tool call, not just command-bearing ones. The capture file is created with restrictive permissions (directory `0700`, file `0600`) since it carries target hosts/users/session IDs and (when opted in) raw command output. Result persistence is opt-in via `APTL_RED_CAPTURE_INCLUDE_RESULT=true` because tool stdout (mimikatz dumps, hashcat cracks, file reads) can carry unlabelled credentials that pattern-based redaction cannot reliably mask. The default capture covers args + outcome (`exit_code`, `signal`, `success`, `duration_ms`, `session_id`). Activity taxonomy in [`docs/red-team-taxonomy.md`](docs/red-team-taxonomy.md) covers port_scan, network_connection, ssh_login_attempt, credential_brute_force, password_cracking, web_attack, web_discovery, host_discovery, remote_execution, network_poisoning, credential_dumping, exploit_framework, and a generic process_execution fallback (the leading executable is always preserved on `aptl.tool`). The classifier handles transparent wrappers (`sudo`, `proxychains[4] -f config`, `nice -n N`, `taskset -c N`, `env`, `time`, `nohup`, `unbuffer`, `stdbuf`, `ionice`), shell wrappers (`bash -c '<inner>'` / `sh -c` / `zsh -c` / `dash -c`), compound shell separators (`&&`, `||`, `;`, `|`, single `&`), redirection forms (`2>&1`, `&>`, `<&3`), and shell-meta tokens (`${VAR}`, `$(cmd)`, backticks, `<(...)`). The extractor produces OCSF endpoint/file/URL/actor objects with per-tool flag semantics — nmap `-p` is a port list, ssh `-p` is a connection port, hydra `-p` is a password, nc `-p` is a local source port; `-l` / `-u` / `-U` map per-tool to either a target user (ssh / hydra / nxc / cme / smbclient / rpcclient / ldapsearch …) or a target URL (`sqlmap -u …`); `-w` is a wordlist for web_discovery / credential_brute_force and a timeout for nc / ncat / socat; `-o` is an output file only for tools that take one (so `ssh -o StrictHostKeyChecking=no` is not promoted to `file.path`); `-J jumphost` for ssh, `-H <hash>` and `-d <domain>` for nxc / cme / crackmapexec / impacket are recognised and consumed. URL extraction is scoped to standalone URL tokens or values of explicit target-URL flags so headers / decoy URLs (`-H 'Referer: https://decoy'`) don't hijack `http_request.url`; URL userinfo and query-string secrets are masked at the structured-field boundary; bracketed IPv6 authorities (`http://[::1]:8080/x`) split correctly and IP literals land in `dst_endpoint.ip`. Cross-cutting redaction lives in `mcp/aptl-mcp-common/src/redaction.ts` so OTel spans and run archives benefit from the same masking: `Authorization:` / `Bearer` / cookies / PEM blocks / URL userinfo / `--password value` / hydra-style `-p value` (with quote-, `=`-, and attached-form awareness; numeric values stay visible for non-credential tools but mask when the segment contains a credential tool — `nmap -p 22 host && hydra -p X host ssh` keeps the port and masks the password) / Samba `username%password` and Basic-auth `user:pass` in `--user` / `-u` / `-U` (URLs are not mistaken for credentials) / NTLM `-H <hash>` for credential tools (curl `-H 'X-Foo: bar'` headers are not redacted because the segment lacks a credential tool). The SIEM logger applies an OCSF time-in-ms timestamp, normalised `status_id` / `status` outcome (with `status_code` carrying the source-specific exit code or signal name), a per-class default severity bumped to MEDIUM on failure, MITRE attack technique/tactic via `attacks[]`, async-aware `SiemSink` contract, and an Unknown-status path for raw-mode session commands whose transport-level `exit_code` is not a real outcome. The new `postToolHook` extension point on `createMCPServer` / `startServer` in `mcp/aptl-mcp-common` is async-aware (`(info) => void | Promise<void>`) and bounded by `postToolHookTimeoutMs` (default 2000 ms) so a slow async sink never blocks command responses. Best-effort throughout: classification, extraction, redaction, and sink failures emit a stderr diagnostic but never break command execution. The existing `kali_redteam` Wazuh syslog path (`config/wazuh_cluster/kali_redteam_rules.xml`) is unchanged — the OCSF stream is additive until the new path proves coverage parity per ADR-027 § Non-Goals. Implements [#162](https://github.com/Brad-Edwards/aptl/issues/162); architectural rationale in [ADR-027](docs/adrs/adr-027-red-team-structured-logging.md). New TypeScript modules in `mcp/mcp-red/src/{classifier,extractor,logger,capture}.ts`. New tests in `mcp/mcp-red/tests/{classifier,extractor,logger,capture}.test.ts` and `mcp/aptl-mcp-common/tests/server-hook.test.ts` plus extensive additions to `mcp/aptl-mcp-common/tests/redaction.test.ts`. Pre-commit hook `vitest-mcp-red` and the `mcp-tests` CI job now exercise mcp-red's vitest suite alongside aptl-mcp-common, and a follow-on CI step rebuilds every dependent MCP (`mcp-wazuh`, `mcp-soar`, `mcp-network`, `mcp-indexer`, `mcp-reverse`, `mcp-threatintel`, `mcp-casemgmt`) against the freshly built common artifact to catch type-level breakage from common API drift.
- Trivy and OSV-scanner are now wired into `.github/workflows/checks.yml` as four advisory CI jobs: `trivy-fs` (filesystem vulnerability + secret scan), `trivy-iac` (Dockerfile / compose / config misconfiguration scan), `trivy-image` (matrix scan over the 14 Dockerfiles under `web/` and `containers/` — including `containers/base/Dockerfile.{rocky,ubuntu}` even though those are inline-copied into kali / victim / workstation / reverse rather than built standalone in production — with a per-entry GHA-backed buildx layer cache), and `osv-scanner` (CVE coverage on the project-root `uv.lock`). All four jobs use `continue-on-error: true` and `--exit-code 0` (or scanner equivalent), pass `limit-severities-for-sarif: true` to Trivy so the SARIF output respects the configured severity filter, and publish output as a short-retention `actions/upload-artifact@v4` artifact (7 days, matching the existing coverage-artifact pattern). The osv-scanner binary is downloaded at a pinned version and verified against the publisher's `osv-scanner_SHA256SUMS` manifest before execution. Workflow token permissions are unchanged — output is artifact-only, not pushed to GitHub code scanning. Implements [#278](https://github.com/Brad-Edwards/aptl/issues/278); architectural rationale in [ADR-026](docs/adrs/adr-026-advisory-ci-vulnerability-scanning.md).
- **Hard per-function complexity gate (`ruff` `C901`).** `ruff` is added as a dev dependency (`uv`-locked to the resolved version), configured **minimally** — `[tool.ruff.lint] select = ["C901"]` and `[tool.ruff.lint.mccabe] max-complexity = 15` (matching SonarCloud's default cognitive-complexity threshold; only `C901` is enabled, so this is a complexity guard, not a style linter). A new `ruff-complexity` pre-commit hook (`ruff check src/`, run first so a god-method fails fast before the test suites; `language: python` with `additional_dependencies: ["ruff>=0.15.0"]` so the hook is self-contained and needs no project `.venv`) makes it a **hard** gate locally and in CI via the existing required "Pre-commit hooks" job — unlike the SonarCloud quality gate, which stays advisory (its "Coverage on New Code ≥ 80%" condition would block infra-only commits) and is new-code-scoped. Four large modules with over-threshold functions (`core/runtime/compiler.py`, `core/runtime/planner.py`, `core/sdl/parser.py`, `core/sdl/validator.py` — worst: `compile_runtime_model` CC 45, `_verify_objectives` CC 43, `_verify_workflows` CC 37) are exempted via `[tool.ruff.lint.per-file-ignores]` for the *whole file* (not per-`def` `# noqa`, which would touch those files and make SonarCloud's PR analysis re-surface their pre-existing debt as "new code") and listed in [ADR-010 § Complexity backlog](docs/adrs/adr-010-sonarcloud-quality.md) — drop a file from the ignore list once `ruff check src/<file>` passes for it without it; ratchet `max-complexity` down as the list shrinks. Architectural rationale in [ADR-010](docs/adrs/adr-010-sonarcloud-quality.md).

### Changed

- `aptl lab start` no longer rewrites checked-in service config in place to inject runtime credentials. The two credentialized Wazuh files under `config/` — `config/wazuh_dashboard/wazuh.yml` (`API_PASSWORD`) and `config/wazuh_cluster/wazuh_manager.conf` (`<cluster><key>` ← `WAZUH_CLUSTER_KEY`) — are now treated as source-owned templates: `core.credentials.sync_dashboard_config` / `sync_manager_config` read them, apply the substitution, and *render* the result to `.aptl/config/wazuh_dashboard/wazuh.yml` and `.aptl/config/wazuh_cluster/wazuh_manager.conf` (the already-gitignored `.aptl/` state tree) under `0700` directories, with an atomic rename and a `0644` file mode — owner-only would break the Wazuh Dashboard image's non-root process reading its bind-mounted config, so the owner-only *directory* is the host-side access control instead. `docker-compose.yml` mounts those rendered copies into the dashboard / manager containers; the other ~18 non-credentialized `config/wazuh_cluster/*` mounts (rules, decoders, certs, helper scripts) are unchanged. The project-root containment guardrail (issue #266) now covers both the source-template path and the rendered-output path. Net effect: a `git checkout` is no longer mutated by lab startup, `git status` stays clean after `aptl lab start`, and the repo never carries live secrets. `aptl lab start` remains the supported entrypoint — a bare `docker compose up` against a fresh checkout will not find the rendered mounts, the same property the gitignored `config/wazuh_indexer_ssl_certs/*` certificate mounts already have. As part of this change, `config/wazuh_cluster/wazuh_manager.conf`'s `<indexer><ssl><key>` element — which had been dirtied to the literal cluster key value by an old blanket-`<key>` startup run and committed (`259c26c`) — is restored to its correct path value `<key>/etc/ssl/filebeat.key</key>` (matching the `wazuh.manager-key.pem:/etc/ssl/filebeat.key` mount and the sibling `<certificate>/etc/ssl/filebeat.pem</certificate>`), its `<cluster><key>` placeholder is reworded from a hex-shaped value to a plain `PLACEHOLDER_REPLACED_AT_RENDER_TIME` marker (it is always replaced at render time, and the hex shape tripped secret scanners), and a `tests/test_credentials.py::TestCheckedInTemplates` guard now rejects any credential-shaped (32-hex) `<key>` value in the checked-in template. Hardening that rides along: the rendered files are mandatory Compose bind-mount sources, so a render failure of any kind — missing template, a substitution that matched zero times (which would otherwise emit a placeholder/stale copy), a directory sitting at the output path, a containment breach — now aborts `aptl lab start` instead of warning and continuing; `API_PASSWORD` and `WAZUH_CLUSTER_KEY` join the `.env` placeholder check (`aptl.utils.placeholders`) so a fresh `.env` copied from `.env.example` fails before any placeholder secret reaches a generated file; the generated-output path is held to a no-symlinked-chain rule (a symlink at `.aptl/config` or the output file — pointing outside the project *or* back at a tracked file — is rejected) on top of plain containment; the temp file used for the atomic write is created with `tempfile.mkstemp` (unpredictable name, `O_CREAT | O_EXCL | O_NOFOLLOW`, mode `0o600`) so a pre-planted predictable `<name>.tmp` symlink can't redirect the secret, and is unlinked if the write or rename fails (the same no-cleanup-on-rename-failure gap is fixed in `services/misp_suricata_sync/rule_writer.py`). Implements [#200](https://github.com/Brad-Edwards/aptl/issues/200); architectural rationale in [ADR-028](docs/adrs/adr-028-runtime-rendered-service-config.md) (which supersedes the "write back into `config/`" part of [ADR-007 § Security Guardrail](docs/adrs/adr-007-python-cli-control-plane.md)). `tests/test_credentials.py` pivots to assert on the rendered output and unchanged source, and adds coverage for restrictive permissions, atomic/idempotent writes, zero-match render errors, output-path symlink rejection (escape *and* back-into-tracked), pre-planted-temp-symlink immunity, and temp-cleanup on rename failure; new `TestSyncCredentialsStep::{test_renders_to_aptl_config_and_leaves_source_untouched,test_missing_template_aborts_lab_start,test_disk_error_aborts_lab_start}` and `TestOrchestrateLabStart::test_aborts_when_credential_render_fails` in `tests/test_lab.py` pin the end-to-end behavior.

### Fixed

- Top-level `aptl.json` config drift was silent: `AptlConfig` allowed unknown keys, so the checked-in `edr_agents` and `agent_configs` sections lived as dead configuration with no runtime consumer, and any future typo at the top level would have been silently accepted the same way. `AptlConfig` now sets `extra="forbid"` (matching every nested model), and the dead `edr_agents` / `agent_configs` blocks have been removed from `aptl.json`. Implements [#190](https://github.com/Brad-Edwards/aptl/issues/190); architectural rationale in [ADR-025](docs/adrs/adr-025-strict-first-party-config-schema.md). New tests in `tests/test_config.py` cover model-level rejection of unknown / dead keys, file-loader rejection via `load_config`, and a regression guard that the checked-in `aptl.json` continues to load cleanly. New `tests/test_cli_config.py::test_validate_fails_on_unknown_top_level_key` pins the CLI surfacing.
- Telemetry/archive paths leaked service credentials and MCP tool arguments. `RangeSnapshot.to_dict()` previously serialized `ServiceEndpoint.credentials` (the lab's default Wazuh dashboard / indexer / API service credentials) directly into `aptl lab status --json` stdout and `--output` files; `traceToolCall` recorded full MCP tool arguments and responses as OTel span attributes shipped to Tempo. Both boundaries now run a shared `redact()` helper that recurses through dicts/lists, traverses string values (parsing JSON-encoded payloads and scanning for inline `Authorization:`, `Bearer`, `<key>=value`, and `--<key> value` patterns), replaces values whose key matches credential tokens (`pass`, `secret`, `token`, `credential`, `authorization`, `cookie`, `jwt`, `bearer`, `api_key`, `apikey`, `key`, `session`) with `[REDACTED]`, and preserves explicitly path-suffixed field names (`key_path`, `key_file`, `public_key`). Implements [#185](https://github.com/Brad-Edwards/aptl/issues/185); architectural guardrail recorded in [ADR-012 § Security Guardrail](docs/adrs/adr-012-opentelemetry-integration.md). New shared helpers `src/aptl/utils/redaction.py` and `mcp/aptl-mcp-common/src/redaction.ts`. `RangeSnapshot.to_dict()` and `traceToolCall()` redact at their respective serialization boundaries so individual call sites never own the policy. New `tests/test_redaction.py` and `mcp/aptl-mcp-common/tests/redaction.test.ts` cover the helper; new redaction assertions in `tests/test_snapshot.py::TestDataclasses` and `mcp/aptl-mcp-common/tests/telemetry.test.ts` pin the boundary behavior, with the TS suite using an in-memory OTel span exporter to assert that recorded attributes contain `[REDACTED]` rather than the secret-shaped synthetic placeholders the tests inject.
- Path-traversal risk in `core.credentials.sync_dashboard_config` / `sync_manager_config` (Sonar BLOCKER S6781 — `new_security_rating=5` on `dev`). Both functions now take `project_dir` plus build the canonical project-relative path internally (`config/wazuh_dashboard/wazuh.yml` and `config/wazuh_cluster/wazuh_manager.conf`) and validate containment via `Path.resolve()` + `is_relative_to` before any I/O. Symlinks under the canonical location pointing outside the project root are rejected with `ValueError`. Implements [#266](https://github.com/Brad-Edwards/aptl/issues/266); architectural guardrail recorded in [ADR-007 § Security Guardrail](docs/adrs/adr-007-python-cli-control-plane.md). `_step_sync_credentials` in `core/lab.py` separates the containment `ValueError` (security guardrail breach — fails lab start with a `LabResult(success=False, …)`) from ordinary sync failures (non-fatal warning). New `TestPathContainment` class in `tests/test_credentials.py` covers symlink-escape rejection and write-to-canonical-only behavior; new `TestSyncCredentialsStep` class in `tests/test_lab.py` pins the failure-class split. Existing credential tests pivoted from the per-test file path to the new (project_dir, secret) shape.
- Pre-existing circular import on a fresh `aptl.core.lab` load (`lab.py` → `snapshot.py` → `aptl.core.deployment` package → `deployment/backend.py` and `deployment/docker_compose.py` re-entering `lab.py` mid-load to grab `LabResult`/`LabStatus`). Extract the two dataclasses to a new leaf module `aptl.core.lab_types`; `aptl.core.lab` re-exports for backward compatibility; deployment backends import directly from the leaf. Surfaced when `tests/test_lab.py::TestSyncCredentialsStep` (added in this PR) was the first thing to load `aptl.core.lab` in isolation.

### Added

- `aptl lab continuity-audit` — orchestrator-side purple-team continuity carve-out. Detects and reverts blanket kali source-IP DROP/REJECT rules on target containers (`-A INPUT -s <kali_ip> -j DROP` with no port/protocol/payload qualifier), preserving granular blue tradecraft. Complements ADR-021's in-band `aptl-firewall-drop` whitelist by catching bypass paths: custom AR scripts, raw iptables in Wazuh manager command fields, and manual researcher mistakes. Audit is stateless and idempotent; events are archived to a specific run's `continuity-events.jsonl` when invoked with `--run-id <existing-run>` (the run must exist in the run store). The session-bound archival path is plumbed but inactive until the runtime engine starts populating `ScenarioSession.run_id` (#263 / RTE-001). Runs unconditionally — APTL is the purple-team lab — with the formal `scenario.mode == PURPLE` gate landing alongside the SDL `mode` field in [#263](https://github.com/Brad-Edwards/aptl/issues/263). Implements [#252](https://github.com/Brad-Edwards/aptl/issues/252); architectural rationale in [ADR-024](docs/adrs/adr-024-orchestrator-side-purple-continuity-carve-out.md). New module `src/aptl/core/continuity.py`, CLI command in `src/aptl/cli/lab.py`, and 38 unit tests plus 3 LIVE_LAB integration tests in `tests/test_continuity.py`. Manual smoke validation: `scripts/test-continuity-audit.sh`.
