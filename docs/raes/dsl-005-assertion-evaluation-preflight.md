...
implementation plan. The assertion contract is owned by RAES (upstream issue
Brad-Edwards/aces#657); APTL consumes that contract and evaluates it against a
realized environment. ADR-035 remains the RAES-adoption decision, ADR-044 owns
run records, and ADR-046 owns dynamic realization and evidence boundaries.

## Architecture Decisions

- RAES remains the sole authority for proposition, assertion, predicate,
  polarity, role, quantifier, truth-outcome, evidence, temporal-context, plan,
  and snapshot shapes. Consume the selected `raes` package's compiled
...
- `AptlEvaluator` owns proposition observation, predicate evaluation, polarity,
  ...
  single sequencing owner remains the RAES handoff in `aptl.backends.raes`.
- Observation is side-effect-free and backend-scoped. It may reuse the current
  ...
  `raes_observation.observe_realization()` concern map: realization-conformance
...
- Capability publication is evidence-backed. `create_aptl_manifest()` may claim
  ...
  actually evaluate. Unsupported inputs produce the RAES `unsupported` outcome
  or planner diagnostics; they are not coerced to `false`.

## Required Cross-Cutting Concerns

- **RAES schema and validation:** `raes.parse_sdl_file`, RAES semantic
...
- **APTL adapter owners:** `create_aptl_runtime_target()`,
  `_apply_execution_plan()`, `AptlEvaluator`, `_raes_evaluator_engine`,
...
- **Realized-state owners:** `AptlProvisioner`, `AptlRealization`, the
  ...
  `RangeSnapshot` capture. RAES adapters do not call Docker directly.
- **Evidence and persistence:** RAES evidence requirements and truth-result
  `evidence_refs`, `LocalRunStore.write_json()` / `write_jsonl()` /
  `append_jsonl()`, the RAES snapshot embedded by `raes_repro`, and the existing
  run id resolved once by lab start. Add references to the existing run record;
  do not create an assertion-run manifest.
- **Errors and observability:** RAES `Diagnostic` with stable code/domain/address,
  `redact()`, `get_logger()`, `render_raes_diagnostics()`, `LabResult`,
...
- **Verification conventions:** extend `tests/test_raes_evaluator.py`,
  `tests/test_raes_orchestrator.py`, `tests/test_workflow_engine.py`,
  `tests/test_raes_backend.py`, `tests/test_raes_repro.py`, and existing
...
| --- | --- |
| SDL parser and semantic validator | Accept only RAES-valid propositions, assertions, role-constrained uses, finite subjects, typed predicates, and evidence requirements. APTL does not parse or structurally revalidate their YAML. |
...
| Runtime target and result shape | Return RAES `ApplyResult` and `RuntimeSnapshot`. Preserve unrelated carriers and pass snapshot-address, result-contract, proposition-truth, transition, and changed-address admission checks in `raes_runtime.backend_calls`. |
...
| Error envelope and logging | Convert probe/contract failures to redacted RAES diagnostics, then existing `LabResult`/startup/API envelopes. Log addresses, codes, outcome classes, and counts; do not log raw evidence, command output, backend stderr, or exception payloads. |
| Persistence and export | Keep truth results in the RAES runtime snapshot and structured evidence under the current run through redacting `LocalRunStore` methods. Path validation remains in runstore; exporter remains packaging-only. Ensure assertion/evaluation evidence is included by the existing run-record evidence-reference enumeration. |
...
or RAES diagnostics), not an APTL truth DTO. Bindings should be selected by the
...
reference must remain explicit so observed decided truth satisfies RAES binding
...
- Dependency/contract authority: `pyproject.toml`, `uv.lock`, and the installed
  RAES parser, processor, runtime, contracts, conformance corpus, and validators.
- Runtime path: `src/aptl/backends/raes.py`, `raes_manifest.py`,
  `raes_evaluator.py`, `_raes_evaluator_engine.py`, `raes_orchestrator.py`,
  `raes_provisioner.py`, `raes_realization*.py`, `raes_observation.py`, and
...
- Lifecycle/output path: `src/aptl/core/lab.py`, `lab_types.py`, `runstore.py`,
  `snapshot.py`, `src/aptl/backends/raes_repro.py`, CLI output, authenticated API
...
- Repo gates: existing evaluator/orchestrator/workflow/backend/repro tests,
  RAES target conformance, static/live scenario gates, `pytest`, and pre-commit.
...
- Do not hand-build truth-result dicts, duplicate RAES polarity/quantifier logic,
...
- Do not define or modify assertion semantics, schemas, controlled vocabulary,
  plan metadata, or truth-result contracts in APTL; upstream RAES is authoritative.
