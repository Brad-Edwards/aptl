...
`raes_backend_libvirt` is not APTL source. It is shipped by the `raes`
...
tests therefore belong in the RAES repository. APTL must consume a released
fix; it must not patch `.venv`, copy the backend in-tree, or add a second
libvirt adapter.

Keep two existing RAES concerns distinct:
...
- **SDL shape and semantics:** `raes.nodes.ServicePort`, `SDLModel`
...
- **Compiler and plan contracts:** the RAES compiler, `RuntimeModel`,
...
- **Provider admission:** `interpret_provisioning_plan()` is the single pure
  ...
  must produce `raes_contracts.diagnostics.Diagnostic` errors before the
...
- **Errors and state:** use stable RAES diagnostic codes, resource addresses,
...
- **APTL adapter:** ADR-035 and ADR-046 remain binding. APTL-facing RAES errors
  continue through `aptl.backends.raes_diagnostics`, including redaction at the
...
protocol, port or range, rule precedence, and default action. Until RAES owns
...
- Do not rely on the current local virtual environment when selecting the RAES
...
- Redesigning RAES `ServicePort`, infrastructure ACL semantics, libvirt
