...
implementation plan. The variable and substitution contract is owned by RAES
...
- RAES remains the sole authority for variable declarations, defaults, types,
  ...
  or consumes a RAES-admitted plan; it does not mutate YAML, interpolate
...
- Keep the lifecycle types distinct: authored `Scenario`, RAES
...
- The selected `aces-sdl` 0.21.x surface exposes instantiation details as free
  ...
  the RAES dependency rather than introducing duplicate APTL validation or an
...
- Runtime bindings are explicit per-run inputs. They do not come implicitly
  ...
  legitimate clean-boot/retry attempt and handed to RAES unchanged.
...
- **RAES contract and validation:** `raes.parse_sdl_file`, source/import
  constraints, semantic validation, RAES instantiation, `RuntimeManager.plan()`,
...
- **APTL admission and lowering:** `start_raes_scenario()`,
...
- **Single-model consumers:** `participant_action_specs_from_runtime_model()`,
  ...
  details, manifest payload, and RAES run evidence. The best-effort
...
- **Configuration and secrets:** strict `AptlConfig`, `.env`/`EnvVars`,
  ...
  systems.
- **Errors and observability:** RAES `Diagnostic` for plan/apply failures,
  `raes_diagnostics`, `render_raes_diagnostics()`, `LabResult`,
...
- **Evidence and persistence:** `LocalRunStore` redacting JSON/JSONL methods,
  `raes_repro`, `RuntimeSnapshot`, and `RangeSnapshot.to_dict()`. Preserve an
...
- **Workflow conventions:** `orchestrate_lab_start()`, `_LabStartContext`,
  `_step_start_containers()`, clean boot, static/live RAES gates, `pytest`, and
...
| RAES parse and semantic admission | Let the selected RAES package parse, shape-check, resolve imports, and validate authored SDL. Do not add an APTL schema or placeholder scanner. |
| RAES instantiation | Pass one explicit mapping with RAES-supported scalar types to `RuntimeManager.plan(parameters=...)`. RAES owns required/default, undeclared, type, constraint, selected-profile, unresolved-reference, and post-substitution checks. |
...
| Provider policy | Re-run all existing image, content, network, port, account, participant-binding, and backend capability checks on concrete values. Successful RAES substitution does not waive provider safety policy. |
...
The required seam is the existing RAES planning call, parameterized by:

`(Scenario, explicit per-run bindings, selected RAES profile, prior
RuntimeSnapshot, RuntimeTarget) -> ExecutionPlan`.

The bindings are handed to RAES exactly once. Every later concern receives the
...
- Contract authority: `pyproject.toml`, `uv.lock`, the selected `raes`
  parser/instantiator/runtime APIs, RAES diagnostics, manifest, conformance
  corpus, and upstream contract tests.
- Runtime path: `src/aptl/backends/raes.py`, `raes_start_model.py`,
  `raes_participant_actions.py`, `raes_participant_bindings.py`,
  `raes_realization.py`, `raes_realization_model.py`,
  `raes_realization_values.py`, `raes_profiles.py`, `raes_provisioner.py`, and
  `raes_diagnostics.py`.
- Provider/security path: `raes_image_realization.py`,
  `raes_content_realization.py`, `raes_realization_networks.py`,
  `raes_account_realization.py`, `src/aptl/core/deployment/`, host-port checks,
...
- Lifecycle/output path: `src/aptl/core/lab.py`, `lab_types.py`, `runstore.py`,
  `snapshot.py`, `src/aptl/backends/raes_repro.py`, CLI lab output,
...
- Test path: `tests/test_raes_backend.py`,
  `tests/test_raes_realization_service_ports.py`,
  `tests/test_raes_repro.py`, `tests/test_lab.py`, CLI/API tests, target
...
- Lab start currently calls `selected_profiles_for_scenario()` after a
  successful RAES start, causing a second raw parse/plan. Use
  `AcesStartOutcome.selected_profiles`; otherwise required bindings disappear
  or defaults may diverge from the executed plan.
- The SOC startup path currently retries any RAES failure after a delay.
...
- `raes_realization_values` currently drops unsubstituted port tokens, and tests
...
- `raes_repro` currently records `scenario_parameters: null` and a note that no
  parameter surface exists. That statement becomes stale, but replacing it
  with raw RAES binding provenance would create a secret/data leakage surface.
...
- Do not conflate RAES `${variable}` semantics with Compose interpolation,
...
- Do not treat a substitution failure as a RAES provisioning diagnostic,
...
- Do not define or change SDL variable syntax, scope, precedence, types,
  ...
  provenance in APTL; the upstream RAES contract is authoritative.
...
- Do not accept arbitrary externally serialized plans unless the selected RAES
