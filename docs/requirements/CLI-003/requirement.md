...
`aptl lab start` shall run its startup steps serially, in the fixed order `_LAB_START_STEPS` defines, while holding the project lifecycle lock. The order shall preserve these invariants:

1. **Admission before preparation.** Environment loading (`.env` hydration and secret validation) and configuration loading (`aptl.json`, the deployment backend, and RAES scenario admission) complete before any host artifact is prepared. Startup then refuses a project whose runtime residue already exists, or whose runtime presence the backend cannot observe.
2. **Pre-flight before realization.** Host port resolution, the SSH control-plane key, host requirements (`vm.max_map_count` and Docker Buildx), credentialized service config, Suricata runtime volumes, Wazuh and SOC certificates, the bind-mount pre-flight, and image pre-pull all run before realization starts any range container. Material that the admitted scenario declares as a generated artifact is produced during realization, not by these host-side steps.
...
4. **Readiness after realization.** Selected Wazuh indexer and manager API services that the backend didn't prove are polled after realization, followed by SSH reachability probes for the interactive targets the realization declares.
5. **Enrichment after readiness.** Terminal SSH host-key pinning, MCP build, SOC seeding, and MCP client config sync run after readiness, with SOC seeding before MCP client config sync.
