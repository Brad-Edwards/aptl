"""Tests for the APTL RAES evaluation adapter (issue #312)."""
...
def test_start_derives_condition_pass_but_objective_stays_unresolved():
    ...
    RAES ADR-073 routes objective success exclusively through
...
def _evaluation_op(address, payload, *, action=None, resource_type="objective"):
    from raes_contracts.planning import ChangeAction, EvaluationOp
...
def test_start_fails_closed_on_scoring_chain_resource():
    ...
    construction time — RAES itself fails closed on the scoring chain one
