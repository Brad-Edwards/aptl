...
def _execution_plan_with_derived_realization_requirements():
    ...
    this processor-substituted value as author-declared. RAES #760 fixed that and
...
def _execution_plan_with_content_realization_requirement():
    """Compile an EXACT file content concern through RAES's public planner."""
    from textwrap import dedent
...
def _drive_plan_through_target(
    ...
    Scenario start now routes through RAES's own ``RuntimeManager.apply``
    ...
    ``start_raes_scenario`` depends on, without pulling in RAES's own SEM-218
    realization gate — that gate is RAES's concern and is exercised directly by
...
class _FakeRuntimeManager:
    """Base test double for ``raes_runtime.manager.RuntimeManager`` (issue #578).
...
def test_current_backend_docs_cover_declared_manifest_components():
    ...
    documentation = " ".join(
        (PROJECT_ROOT / "docs" / "raes" / "techvault-static-validation-gate.md")
        .read_text()
        .split()
    )
...
@pytest.mark.parametrize(
    "relative_path",
    [
        "docs/raes/dsl-008-realization-preflight.md",
        "docs/raes/dsl-010-participant-runtime-preflight.md",
        "docs/raes/orchestration-capable-profile-preflight.md",
        "docs/raes/techvault-live-validation-preflight.md",
        "docs/raes/techvault-static-validation-preflight.md",
    ],
)
...
def test_assert_compiled_addresses_rejects_compiled_but_unassigned_refs():
    ...
    from aptl.backends.raes_participant_bindings import _assert_compiled_addresses
...
def test_start_helper_returns_specs_from_compiled_scenario(mocker):
    ...
    spec_mock = mocker.patch(
        "aptl.backends.raes_participant_actions."
        "participant_action_specs_from_runtime_model",
...
def test_compose_alias_helpers_cover_string_builds_and_alpine_images():
    from aptl.backends.raes_profiles import _build_aliases, _image_aliases
...
def test_container_name_and_single_value_helpers_handle_ambiguous_inputs():
    from aptl.backends.raes_profiles import ComposeProfileIndex
    from aptl.backends.raes_realization import _container_name
    from aptl.backends.raes_realization_model import _single_or_none
...
def test_start_raes_scenario_uses_parser_runtime_manager_and_backend(
    mocker,
    tmp_path,
):
...
def test_start_raes_scenario_passes_runtime_parameters_to_raes_planner(
    ...
    assert interaction_resolver.call_count == 1


def test_start_raes_scenario_threads_resolved_run_target(mocker, tmp_path):
    from aptl.backends import raes
    ...
    mocker.patch("aptl.backends.raes.RuntimeManager", FakeRuntimeManager)
    mocker.patch(
        "aptl.backends.raes.participant_action_specs_from_runtime_model",
        return_value={},
    )
    ...
    run_plan = mocker.patch(
        "aptl.backends.raes._run_execution_plan", return_value=outcome
    )
    store = object()
...
def test_start_raes_scenario_projects_instantiation_failure_without_values(
    tmp_path,
):
    from aptl.backends import raes
...
def test_start_raes_scenario_retries_soc_apply_without_replanning(mocker, tmp_path):
    from aptl.backends import raes
...
def _workflow_and_evaluation_execution_plan():
    """Compile a minimal workflow+objective scenario into its full RAES plan.
    ...
    *real* compiled evaluation plan (with a genuine ``result_contract``, not a
    ...
    from raes_processor.compiler import compile_runtime_model
    from raes_processor.planner import plan as raes_plan
    from raes.parser import parse_sdl
    ...
    return raes_plan(compile_runtime_model(scenario), create_aptl_manifest())


def _workflow_orchestration_plan():
    """Compile a minimal workflow scenario into its RAES orchestration plan."""
    return _workflow_and_evaluation_execution_plan().orchestration


def test_start_raes_scenario_submits_orchestration_for_workflow_scenario(
    mocker, tmp_path
):
    ...
    ``RuntimeControlPlane`` loop; it applies through RAES's own
...
def test_start_raes_scenario_fails_when_provisioning_backend_fails(mocker, tmp_path):
...
def test_start_raes_scenario_fails_when_orchestration_fails(mocker, tmp_path):
    """An orchestration-phase failure (provisioning already applied) surfaces as
    a failed lab start, not a silent success."""
    from raes_contracts.planning import ChangeAction, OrchestrationOp, OrchestrationPlan
...
def test_start_raes_scenario_submits_evaluation_for_objective_scenario(
    mocker, tmp_path
):
...
def test_start_raes_scenario_drives_workflows_after_registration(mocker, tmp_path):
    ...
    def fake_create_target(
        *,
        ...
        target = raes.RuntimeTarget(
            name=raes.APTL_RAES_TARGET_NAME,
    ...
    mocker.patch("aptl.backends.raes.RuntimeManager", FakeRuntimeManager)
    mocker.patch("aptl.backends.raes.create_aptl_runtime_target", fake_create_target)
    backend = MagicMock()
    backend.realize.return_value = LabResult(success=True, message="ok")
    config = AptlConfig(lab={"name": "test"}, containers={"victim": True})
...
def test_start_raes_scenario_fails_closed_on_provisioning_plan_error(mocker, tmp_path):
...
def test_apply_failure_reattaches_backend_diagnostics_after_gate_flood(
    ...
    When the backend apply fails, ``raes_runtime`` gates the unrealized
    ...
    mocker.patch("aptl.backends.raes.RuntimeManager", MaskingRuntimeManager)
    target = MagicMock()
...
def test_provisioner_realization_details_follow_distinct_plan_content(tmp_path):
    ...
    # each as a dict (raes 0.21.0 removed the untyped shape).
...
def test_provisioner_rejects_unsupported_resource_type(tmp_path):
    """APTL's ``SUPPORTED_RESOURCE_TYPES`` is now the *entire* RAES-allowed
    ...
    outside that same vocabulary at construction time. RAES itself now fails
...
def test_raes_backend_does_not_import_legacy_sdl_parser():
    raes_module = Path(__file__).resolve().parents[1] / "src/aptl/backends/raes.py"
    source = raes_module.read_text()
...
def test_drive_workflows_receives_threaded_run_store_and_run_id(tmp_path):
    ...
    submission helper) is gone; scenario start now applies through RAES's own
    ``RuntimeManager.apply`` and threads run_store/run_id via
    ``_drive_orchestrator_workflows`` instead (issue #578).
    """
    from unittest.mock import MagicMock as _MagicMock

    from aptl.backends.raes import AptlOrchestrator, _drive_orchestrator_workflows
...
def _apply_disclosure_scenario(tmp_path, backend, execution_plan=None):
    """Apply a SEM-218 fixture scenario through RAES's own runtime manager.
    ...
    only path on which RAES actually runs its non-approximation gate and attaches
...
def test_apply_provisioning_discloses_author_declared_provenance(tmp_path):
    ...
    Pins the provenance *vocabulary* to RAES's own ``ExplicitnessProvenance``
    rather than an APTL-local enum, and pairs with the processor-derived case
    below: the two values must be distinguishable, which is exactly what RAES
    #760 fixed upstream.
    """
...
def test_apply_provisioning_discloses_processor_derived_provenance(tmp_path):
    ...
    is precisely the bug RAES #760 fixed — and precisely what the SEM-218
    disclosure surface exists to keep honest.

    This assertion is unsatisfiable on raes 0.19.1: ``PROCESSOR_DERIVED`` was
