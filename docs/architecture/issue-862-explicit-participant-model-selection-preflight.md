...
| Credentials and environment | ADR-052, the existing workbench credential owner, `contains_placeholder()`, the adapters' minimal base environments, and ADR-029. Resolve only the configured source and deliver only the selected provider's required credential contract; model identity is configuration, not a credential alias. |
...
| Secret and environment binding | Credentials continue through the ADR-052 configured-source boundary and placeholder checks into a minimal provider-supported delivery channel. Do not inherit project/user/provider environment or product login state. Do not place credentials, organization/project ids, or secret-derived digests in model configuration or evidence. |
