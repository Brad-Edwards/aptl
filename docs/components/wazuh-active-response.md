...
1. Create a non-Domain-Admin AD account (for example, `disabletest@TECHVAULT.LOCAL`):
   ```
...
4. Trigger a level-10 rule that names `disabletest` (for example, 6 failed logins):
   ```
