...
def test_native_configs_keep_manual_servers_and_provider_settings(tmp_path):
    ...
    assert (
        codex["mcp_servers"]["aptl-seat-1-red"]
        == {
    ...
    assert "30222" in args and "2222" not in args
...
def test_unchanged_owned_entries_update_but_manual_edits_conflict():
    ...
    assert (
        tomllib.loads(render_codex(original_toml, new, previous=old))["mcp_servers"]
        == {
    ...
    with pytest.raises(ValueError, match="conflict"):
        render_codex(original_toml.replace("old", "manual-edit"), new, previous=old)
...
def test_project_publication_refuses_symlinked_client_file(tmp_path):
    ...
    with pytest.raises(ValueError):
        publish_client_config(tmp_path, "claude", access_record(), {})
