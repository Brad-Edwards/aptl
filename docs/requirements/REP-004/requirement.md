...
Without background traffic, attack traffic is the only traffic on the network — unrealistic and trivially detectable. DETERLab's ExFiles and CyberVAN's traffic generation demonstrate the pattern. Background traffic is essential for realistic detection validation.
