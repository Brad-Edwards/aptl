...
# The guest reads these over a read-only share; both are small documents.
_MAX_DOCUMENT_BYTES = 1024 * 1024


class SeatLaunchError(ValueError):
    """One guest launch could not be authenticated."""
...
def verify_seat_launch(
    descriptor_path: Path,
) -> tuple[SeatLaunchDescriptor, ApplianceBoundaryPolicy]:
    """Authenticate the guest launch from the read-only launch mount.

    The guest is handed two documents: this descriptor and the boundary policy
    the host bound the launch to.  The descriptor carries that policy's digest,
    so the guest checks the bytes it was given rather than trusting the file it
    happens to find beside them.
    """

    try:
        payload = descriptor_path.read_bytes()
    except OSError as exc:
        raise SeatLaunchError("appliance launch descriptor is unreadable") from exc
    ...
    policy_path = descriptor_path.with_name("boundary-policy.json")
    try:
        policy_bytes = policy_path.read_bytes()
    except OSError as exc:
        raise SeatLaunchError("appliance boundary policy is unreadable") from exc
    if len(policy_bytes) > _MAX_DOCUMENT_BYTES:
        raise SeatLaunchError("appliance boundary policy exceeds its limit")
    actual = "sha256:" + hashlib.sha256(policy_bytes).hexdigest()
    if actual != descriptor.boundary_policy_digest:
        raise SeatLaunchError("appliance boundary policy does not match the launch")
    try:
        policy = ApplianceBoundaryPolicy.model_validate(loads_strict(policy_bytes))
    except ValueError as exc:
        raise SeatLaunchError("appliance boundary policy is invalid") from exc
