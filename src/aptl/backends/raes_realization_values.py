"""Field extraction helpers for RAES-to-APTL realization."""
...
PLACEMENT_TARGET_KEYS = {
    ...
    "domain-controller-placement": ("target_address", "node_name"),
...
def resource_name(address: str, payload: Mapping[str, Any]) -> str:
    """Return a resource display name, defaulting to its RAES address."""
...
def service_ports(
    ...
    RAES carries SDL ``nodes.<n>.services`` into the compiled node payload at
...
def published_ports(
    ...
    RAES carries SDL ``nodes.<n>.runtime.network.published_ports`` into the
...
def _port_number(value: object) -> int | None:
    ...
    RAES types a port as ``int | str`` so an authored ``${var}`` survives
...
def _protocol(value: object) -> str:
    """Return a normalized transport protocol, defaulting to RAES's own ``tcp``."""
...
def _static_assignments_from_properties(
    ...
    """Extract static assignments from RAES infrastructure properties."""
...
def _static_assignments_from_property_item(
    ...
    """Extract static addresses from one RAES network-property mapping."""
...
def placement_spec(payload: Mapping[str, Any]) -> Mapping[str, Any] | None:
    """Return the compiled RAES resource spec (the Content/Account dump)."""
