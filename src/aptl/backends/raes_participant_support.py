"""Pure helpers for the APTL RAES participant runtime adapter.

These functions carry no runtime state; they are factored out of
``raes_participant_runtime`` so the adapter module stays focused on the
...
def binding_post_state_digest(request: ParticipantActionAdmissionRequest) -> str:
    ...
    Mirrors the RAES reference contract: when the caller does not supply a
