...
- The HTTPS listener is bound on container TCP 5601 and host-published on 127.0.0.1:443:5601 (loopback-only). Unauthenticated `/api/status` returns HTTP 401 JSON; unauthenticated `/` redirects to `/app/login?`.
