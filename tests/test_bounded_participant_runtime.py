"""Participant runtime tests for the bounded RAES v2 loop."""
...
def test_managed_provider_compacts_large_surface_and_maps_exact_candidate(
    ...
    assert len(adapter.messages[0]) <= 64_000
...
def test_head_endpoint_probe_uses_curl_head_semantics() -> None:
    ...
    assert all(command[-1] == "http://aptl-webapp:8080/" for command in curl_calls)
