...
:func:`aptl.backends.raes_materializer.plan_node_materialization`) against a
...
backend failure is translated at this admission boundary into the existing RAES
...
from aptl.backends.raes_diagnostics import diagnostic, render_raes_diagnostics
...
def materialize_node(
    ...
    if diagnostics:
        return LabResult(success=False, error=render_raes_diagnostics(diagnostics))
    return None
