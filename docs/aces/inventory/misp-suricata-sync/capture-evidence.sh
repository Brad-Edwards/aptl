...
# rewrites /var/lib/suricata/rules/misp/misp-iocs.rules on the bind-mounted
# host directory, and triggers a Suricata rule reload over the shared
# unix-command socket volume. It exposes no inbound listener — it dials OUT to
# MISP and writes files/sockets.
#
