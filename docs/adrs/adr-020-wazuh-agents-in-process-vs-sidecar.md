...
The targets do **not** declare `depends_on: { wazuh.manager: service_healthy }`. `wazuh.manager` is in the `wazuh` profile and the targets live in `enterprise` / `dns` / `fileshare`—a hard dependency would make `aptl lab start --profile enterprise` fail when SOC isn't selected. The bootstrap script's 180s wait for the manager auth port and its agent-auth retry loop (12 × 10s) handle the startup-ordering case without coupling profiles.
...
- **Postgres carve-out becomes load-bearing.** If a future attack scenario requires AR on db (for example, rate-limit a credential-spray source), the carve-out blocks it. Mitigation: track the postgres-with-agent custom-image work as a separate issue; this ADR documents that it's a known gap.
...
- **`<auth><force><enabled>yes</enabled>...` lets a compromised container impersonate any agent.** The manager-side force replacement is required for the in-process takeover from a sidecar (same agent name, new IP), but it also lowers the bar for an attacker who reaches the manager's auth port (1515): they can call `agent-auth -A aptl-webapp-agent` from a rogue container, replace the legitimate agent's key, and ship forged telemetry under that name. The lab's threat model accepts this: (a) all four protected docker networks are `internal: true`, so 1515 is only reachable by other lab containers—the attack requires prior compromise of a lab container; (b) `<use_password>no</use_password>` is the default lab posture (no auth secret needed) which is already permissive in the same way; (c) for purple-team scenarios this is acceptable—the SIEM logs the re-registration event and a real SOC would investigate. For production deployments, set `<use_password>yes</use_password>` with per-agent passwords + tighter `<force>` time windows or disable force entirely and rotate via a manual procedure.
