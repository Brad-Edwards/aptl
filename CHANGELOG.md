...
- `docs/components/default-defensive-posture.md` — captures the lab's intentionally-weak starting posture across Suricata, Wazuh detection rules, Wazuh active-response, MISP IOCs, Shuffle, TheHive, AD identity controls, network segmentation, and the (forward-looking) scenario-mode contract. Tags every surface as either `WEAKNESS BY DESIGN` (target's intentional vulnerability — red exploits, blue hardens) or `STARTING POSTURE` (defensive tool wired but disabled — blue enables per iteration) so researchers can distinguish target weaknesses from blue tools that ship switched off. Linked from `docs/index.md`. Documents [#251](https://github.com/Brad-Edwards/aptl/issues/251); references SOC-001, SOC-002, SOC-006, SIEM-005, SIEM-006, SCN-001 and ADRs 019/021/022. Companion follow-up [#263](https://github.com/Brad-Edwards/aptl/issues/263) tracks the SCN-001 vocabulary mismatch (the requirement statement references a `mode` field that doesn't exist in the post-refactor SDL).
