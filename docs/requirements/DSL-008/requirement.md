...
- IMPLEMENTS → GITHUB_ISSUE `956` (Contain faithful realization of SDL privilege and runtime security requirements)
...
- IMPLEMENTS → CODE_FILE `src/aptl/core/runtime_authority_policy.py` (Strict immutable-pack operator grants and no-follow policy loading)
...
- IMPLEMENTS → CODE_FILE `src/aptl/core/deployment/ssh_compose.py` (Policy-bound endpoint/inventory qualification without promoting empty-daemon evidence to containment)
- IMPLEMENTS → DOCUMENTATION `docs/architecture/issue-956-sdl-runtime-authority-containment-preflight.md` (Runtime-authority containment boundary and supported backend profiles)
- TESTS → TEST `tests/test_runtime_materialization.py` (Qualified-profile contract, unsupported materialization, policy, lowering, ordering, effective-model, and escape-surface coverage)
...
- TESTS → TEST `tests/test_deployment_backend.py` (Strict deployment policy pointer and backend construction coverage)
