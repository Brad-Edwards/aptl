...
| `aptl.tool_name` | string | MCP tool name (for example `kali_run_command`) |
...
- `actor.user.name`: `target_user` from the extractor (for example `ssh user@host`,
...
- **IPv4**: bare addresses with octet validation. Out-of-range octets
  (for example `999.0.0.1`) are rejected.
...
- **`-w <path>` / `--wordlist <path>` / `-o <path>` / `--output <path>`**: populates `file.path`.
