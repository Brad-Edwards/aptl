...
- **Hard per-function complexity gate (`ruff` `C901`).** `ruff` is added as a dev dependency (`uv` lock entry pinned to the resolved version), configured **minimally** — `[tool.ruff.lint] select = ["C901"]` and `[tool.ruff.lint.mccabe] max-complexity = 15` (matching SonarCloud's default cognitive-complexity threshold; only `C901` is enabled, so this is a complexity guard, not a style linter). A new `ruff-complexity` pre-commit hook (`ruff check src/`, run first so a god-method fails fast before the test suites) makes it a **hard** gate locally and in CI via the existing required "Pre-commit hooks" job — unlike the SonarCloud quality gate, which stays advisory (its "Coverage on New Code ≥ 80%" condition would block infra-only commits) and is new-code-scoped. The eight functions currently over the threshold are carved out **at the site** with `# noqa: C901  # complexity backlog — split; see ADR-010` (worst: `runtime/compiler.py::compile_runtime_model` CC 45, `sdl/validator.py::_verify_objectives` CC 43, `_verify_workflows` CC 37) and listed in [ADR-010 § Complexity backlog](docs/adrs/adr-010-sonarcloud-quality.md) as the standing refactor backlog — peel them off one PR at a time and ratchet `max-complexity` down. Also fixes a malformed `# noqa: delayed import for mocking` in `core/lab.py` that `ruff` flagged (now `# noqa: PLC0415`). Architectural rationale in [ADR-010](docs/adrs/adr-010-sonarcloud-quality.md).
