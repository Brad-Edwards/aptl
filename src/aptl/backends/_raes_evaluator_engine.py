"""Outcome resolution helpers for the APTL RAES evaluator adapter."""
...
from aptl.backends._raes_evaluator_progress import append_progression
...
class _EvaluationOutcome(object):
    """Resolved evaluator state before it is rendered as RAES DTO payloads."""
...
def evaluation_diagnostic(code: str, address: str, message: str) -> Diagnostic:
    """Build a redacted evaluation-domain RAES error diagnostic."""
    return Diagnostic(
        code=code,
    ...
    if not isinstance(result_contract_payload, dict):
        diagnostics = [
            evaluation_diagnostic(
                ...
                evaluation_address,
                "RAES evaluation resource is missing its compiled result_contract.",
            )
        ]
    else:
        try:
            ...
            diagnostics = [
                evaluation_diagnostic(
                    ...
                    evaluation_address,
                    f"RAES evaluation result_contract is invalid: {exc}",
                )
            ]
...
def _resolve_outcome(
    ...
    """Resolve an operation outcome according to its RAES resource type."""
