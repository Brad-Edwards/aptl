...
- Plans that change any Compose asset (`docker-compose*.yml`), container
  Dockerfiles, or `config/` files MUST validate that a clean `aptl lab stop -v
  && aptl lab start` produces a healthy lab on a fresh machine — the lab is
  the primary product, not the codebase.
