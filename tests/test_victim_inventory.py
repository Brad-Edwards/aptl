"""Checks for the SCN-010 victim steady-state inventory bundle."""
...
VICTIM_DIR = PROJECT_ROOT / "docs" / "aces" / "inventory" / "victim"
VICTIM_DOC_PATH = VICTIM_DIR / "README.md"
CAPTURE_SCRIPT_PATH = VICTIM_DIR / "capture-evidence.sh"
LEDGER_PATH = VICTIM_DIR / "mapping-ledger.yaml"
EVIDENCE_DIR = VICTIM_DIR / "evidence"
...
ROOTFS_LAYER_COUNT = 27
SOURCE_INPUT_COUNT = 11
...
def test_victim_inventory_note_declares_scope_and_evidence():
    text = VICTIM_DOC_PATH.read_text(encoding="utf-8")
    required = (
        "SCN-010",
        "issue #337",
        "aptl-victim",
...
def test_victim_mapping_ledger_validates_without_gap_triage():
    result = validate_mapping_ledger(VICTIM_DIR)
    ...
    assert dispositions["victim.runtime.service-manager-state"] == "encoded"
...
def test_victim_gap_report_has_no_remaining_aces_gaps():
    report = gap_report(VICTIM_DIR)
    assert report["gaps"] == []
    assert report["triage_needed"] == []


def test_victim_evidence_bundle_files_are_present_and_non_empty():
...
def test_victim_evidence_sha256_manifest_matches_files():
...
def test_victim_mapping_ledger_references_every_evidence_file():
    ledger = load_mapping_ledger(LEDGER_PATH)
    refs = set()
    refs.update(
        ref["path"]
...
def test_victim_container_runtime_state_and_identity():
    ...
    assert "aptl_victim_logs:/var/log:rw" in container["HostConfig"]["Binds"]
    assert container["NetworkSettings"]["Networks"]["aptl_aptl-internal"][
        "IPAddress"
    ] == "172.20.2.20"


def test_victim_image_identity_and_source_package_are_recorded():
    ...
    assert "514/tcp" in image["Config"]["ExposedPorts"]
    assert len(image["RootFS"]["Layers"]) == ROOTFS_LAYER_COUNT

    source_checksums = (EVIDENCE_DIR / "source-checksums.txt").read_text(
        encoding="utf-8"
    )
    for source_path in (
        "containers/victim/Dockerfile",
        "containers/victim/entrypoint.sh",
        "containers/victim/install-all.sh",
...
def test_victim_runtime_baseline_captures_expected_steady_state():
    text = (EVIDENCE_DIR / "runtime-baseline.txt").read_text(encoding="utf-8")
    required = (
...
def test_victim_trivy_vulnerability_summary_matches_list():
    counts = {
...
def test_victim_sbom_toolchain_evidence_is_cyclonedx():
...
def test_victim_osquery_evidence_records_requested_tables_and_limits():
    expected_tables = {
        "apt_sources",
        "docker_containers",
        "docker_images",
    ...
    assert {"systemd", "systemd-journal", "rsyslogd", "sshd"} <= process_names
    ...
    assert docker_containers["rows"][0]["name"] == "/aptl-victim"

    docker_images = _json_file("osquery-docker-images.json")
    assert docker_images["status"] == "captured"
    assert any("aptl-victim:latest" in row["tags"] for row in docker_images["rows"])
...
def test_techvault_sdl_encodes_victim_inventory_surfaces():
    data = _yaml_file(TECHVAULT_SDL_PATH)
    victim = data["nodes"]["victim"]
    runtime = victim["runtime"]
    build = victim["source"]["build"]

    assert victim["source"]["name"] == "aptl-victim"
    assert victim["source"]["version"] == IMAGE_DIGEST
    assert build["base_image"] == "rockylinux:9"
    assert build["dockerfile_path"] == "containers/victim/Dockerfile"
    assert len(build["instructions"]) == DOCKERFILE_INSTRUCTION_COUNT
    ...
    assert {
        ...
        key: value
        for key, value in victim["services"][0].items()
        if key != "description"
    }
    assert victim["vulnerabilities"] == ["victim-nopasswd-sudo"]
    ...
    assert len(runtime["local_identity"]["sudo_rules"]) == 1
    assert len([name for name in data["content"] if name.startswith("victim-")]) == (
        VICTIM_CONTENT_COUNT
    )
    assert len([name for name in data["accounts"] if name.startswith("victim-local-")]) == (
        LOCAL_IDENTITY_USER_COUNT
    )


def test_victim_service_manager_units_match_systemd_evidence():
    data = _yaml_file(TECHVAULT_SDL_PATH)
    units = {
        unit["unit_id"]: unit
        for unit in data["nodes"]["victim"]["runtime"]["service_manager_units"]
    }
    ...
    assert rsyslog["active_state"] == "active"
    assert rsyslog["sub_state"] == "running"
    assert rsyslog["result"] == "success"
    assert rsyslog["main_pid"] == 54
...
def test_victim_filesystem_tree_is_encoded_as_runtime_inventory():
    data = _yaml_file(TECHVAULT_SDL_PATH)
    filesystem = {
        entry["path"]: entry
        for entry in data["nodes"]["victim"]["runtime"]["filesystem_inventory"]
    }
    observed_paths = {
...
def test_victim_passwd_users_are_encoded_as_accounts():
    ...
    account_usernames = {
        ...
        if name.startswith("victim-local-")
    }
    passwd_usernames = {
...
def test_victim_runtime_local_identity_matches_passwd_and_group_evidence():
    data = _yaml_file(TECHVAULT_SDL_PATH)
    local_identity = data["nodes"]["victim"]["runtime"]["local_identity"]
    ...
    assert set(sudo_rules) == {"labadmin"}
    assert sudo_rules["labadmin"]["nopasswd"] is True


def test_victim_runtime_network_matches_docker_evidence():
    data = _yaml_file(TECHVAULT_SDL_PATH)
    network = data["nodes"]["victim"]["runtime"]["network"]
    container = _json_file("docker-inspect.container.json")[0]
...
def test_techvault_sdl_parses_and_compiles_with_victim_runtime_fields():
...
def test_parity_inventory_cites_victim_inventory_and_profile_cutover():
    inventory = _yaml_file(PARITY_PATH)
    rows = {row["id"]: row for row in inventory["rows"]}

    row = rows["scen.techvault.victim-inventory"]
    assert row["legacy_source"] == "scenarios/techvault.sdl.yaml"
    assert row["category"] == "aces_sdl"
    assert row["blocking_followup"] == "n/a"
    assert "docs/aces/inventory/victim/" in row["validation_evidence"]
    assert "tests/test_victim_inventory.py" in row["validation_evidence"]
