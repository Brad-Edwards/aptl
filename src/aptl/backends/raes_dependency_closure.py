"""Dependency closure support for RAES-backed APTL realization."""
...
from aptl.backends.raes_realization_model import NetworkRealization, NodeRealization
        ...
        if len(matches) > 1:
            diagnostics.append(
                diagnostic(
                    ...
                    node.address,
                    (
                        "RAES node aliases match multiple APTL Compose "
...
def _selected_node_payloads(
    ...
    """Yield selected RAES node resources with mapping payloads."""
...
def _append_dependency_unresolved_diagnostic(
    ...
    diagnostics.append(
        diagnostic(
            ...
            resource_address,
            (
                "RAES node dependency does not map to an APTL "
...
def _append_dependency_ambiguous_diagnostic(
    ...
    diagnostics.append(
        diagnostic(
            ...
            resource_address,
            (
                "RAES node dependency maps to multiple APTL "
                "Compose services: "
...
def _node_dependency_values(
    ...
    """Return dependency references declared by one planned RAES node."""
...
def _network_reference_aliases(networks: list[NetworkRealization]) -> set[str]:
    """Return normalized RAES network aliases that are not service dependencies."""
...
def _is_network_dependency(dependency: str, network_aliases: set[str]) -> bool:
    """Return whether a dependency reference names a RAES network resource."""
...
def _dependency_reference_aliases(reference: str) -> set[str]:
    """Return normalized aliases for a RAES dependency reference."""
