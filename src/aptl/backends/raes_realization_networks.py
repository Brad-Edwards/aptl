"""Network topology validation for RAES-to-APTL realization."""
    ...
    if parsed_network is not None and parsed_address not in parsed_network:
        diagnostics.append(
            diagnostic(
                ...
                node.address,
                "RAES node static address is outside the declared network CIDR.",
            )
        )
...
def _append_unlinked_static_address_diagnostic(
    ...
    diagnostics.append(
        diagnostic(
            ...
            node.address,
            (
                "RAES node static address references a network "
    ...
    if prior_owner is not None and prior_owner != node.address:
        diagnostics.append(
            diagnostic(
                ...
                node.address,
                "RAES node static address duplicates another node.",
            )
        )
    else:
...
def _append_invalid_cidr_diagnostic(
    ...
    diagnostics.append(
        diagnostic(
            ...
            network.address,
            "RAES network CIDR is not a valid IPv4 network.",
        )
    )
    ...
    if parsed_network is not None and parsed_gateway not in parsed_network:
        diagnostics.append(
            diagnostic(
                ...
                network.address,
                "RAES network gateway is outside the declared CIDR.",
            )
        )
...
def _append_invalid_gateway_diagnostic(
    ...
    diagnostics.append(
        diagnostic(
            ...
            network.address,
            "RAES network gateway is not a valid IPv4 address.",
        )
    )
...
def _append_invalid_static_address_diagnostic(
    ...
    diagnostics.append(
        diagnostic(
            ...
            node_address,
            "RAES node static address is not a valid IPv4 address.",
        )
    )
        ...
        if prior is not None and prior.address != network.address:
            diagnostics.append(
                diagnostic(
                    ...
                    network.address,
                    "RAES network names normalize to the same backend network.",
                )
            )
        else:
            index[key] = network
