"""Checks for the SCN-010 thehive-es steady-state inventory bundle."""
...
ASSET_DIR = PROJECT_ROOT / "docs" / "aces" / "inventory" / "thehive-es"
...
SDL_FILESYSTEM_ENTRY_COUNT = 526
...
REQUIRED_EVIDENCE_FILES = {
    "capture-limits.txt",
    "captured-at-utc.txt",
    "compose-service.thehive-es.json",
    ...
    "docker-logs.thehive-es.txt",
    "docker-network.aptl-security.json",
    "docker-top.txt",
    "docker-version.json",
    "docker-volume.thehive_es_data.json",
    "elasticsearch-state.txt",
    ...
    "syft-version.json",
    "thehive-es-index-mappings.json",
...
def test_thehive_es_note_declares_scope_and_realization_caveats():
    ...
    required = (
        ...
        "nodes.techvault.thehive-es",
        "_cluster/state/metadata",
        "xpack.security",
    )
    missing = [needle for needle in required if needle not in text]
    assert not missing, f"thehive-es inventory note missing scope markers: {missing}"


def test_thehive_es_capture_script_pins_toolchain_and_probes():
    ...
    required = (
        ...
        "elasticsearch-state.txt",
        "_cluster/state/metadata/.geoip_databases",
        "filesystem-tree.txt.gz",
        "evidence-sha256sums.txt",
    )
    missing = [needle for needle in required if needle not in text]
...
def test_thehive_es_mapping_ledger_validates_without_gaps():
    ...
    assert ledger["asset"]["aptl_issue"] == 352
    ...
    assert dispositions["thehive-es.datastore.elasticsearch-service"] == "encoded_with_caveat"
    assert dispositions["thehive-es.runtime.service-listeners"] == "encoded"


def test_thehive_es_gap_report_has_no_remaining_aces_gaps():
    report = gap_report(ASSET_DIR)
    assert report["gaps"] == []
    assert report["triage_needed"] == []


def test_thehive_es_evidence_bundle_files_are_present_and_non_empty():
...
def test_thehive_es_evidence_sha256_manifest_matches_files():
...
def test_thehive_es_mapping_ledger_references_every_evidence_file():
    ledger = load_mapping_ledger(LEDGER_PATH)
    refs = set()
...
def test_thehive_es_trivy_counts_match_severity_breakdown():
    ...
    assert counts == {"HIGH": 23, "MEDIUM": 55, "LOW": 16}
    assert sum(counts.values()) == TRIVY_FINDING_COUNT


def test_thehive_es_listeners_are_rest_and_transport():
...
def test_techvault_sdl_encodes_thehive_es_node(legacy_scenario):
    node = legacy_scenario["nodes"]["thehive-es"]
    runtime = node["runtime"]

    assert node["source"]["name"] == "docker.elastic.co/elasticsearch/elasticsearch"
    assert node["source"]["version"] == IMAGE_DIGEST
    assert node["os_version"] == "Ubuntu 20.04.6 LTS"
...
def test_techvault_sdl_encodes_thehive_es_datastore(legacy_scenario):
    ds = legacy_scenario["nodes"]["thehive-es"]["runtime"]["datastore_services"][0]
    assert ds["engine"] == "elasticsearch"
...
def test_techvault_sdl_compiles_with_thehive_es_runtime_fields(compiled_runtime_model):
    node = compiled_runtime_model.node_deployments["provision.node.techvault.thehive-es"].spec["node"]
    runtime = node["runtime"]
    assert node["source"]["version"] == IMAGE_DIGEST
    assert len(runtime["packages"]) == RUNTIME_PACKAGE_COUNT
    assert len(runtime["package_vulnerabilities"]) == TRIVY_FINDING_COUNT
...
def test_parity_inventory_cites_thehive_es_inventory():
    import yaml

    rows = {row["id"]: row for row in yaml.safe_load(PARITY_PATH.read_text(encoding="utf-8"))["rows"]}
    row = rows["scen.techvault.thehive-es-inventory"]
    assert row["category"] == "aces_sdl"
    assert row["blocking_followup"] == "n/a"
    assert "nodes.techvault.thehive-es" in row["aces_target"]
    assert "docs/aces/inventory/thehive-es/" in row["validation_evidence"]
    assert "tests/test_thehive_es_inventory.py" in row["validation_evidence"]
