...
- IMPLEMENTS → CODE_FILE `src/aptl_techvault/capture_registrations.py` (TechVault's admitted collector registrations and observation offers)
...
- IMPLEMENTS → GITHUB_ISSUE `Brad-Edwards/aptl#992` (Backend-owned evidence capture with RAE scope semantics)
- IMPLEMENTS → CODE_FILE `src/aptl/core/experiment/capture_plan.py` (Canonical RAES demand compilation, exact offer matching, and immutable apparatus admission)
- IMPLEMENTS → CODE_FILE `src/aptl/backends/raes_observability_scope.py` (Closed-scope prohibition and minimum-intrusion selection for open scopes)
- IMPLEMENTS → CODE_FILE `src/aptl/backends/raes_evidence_acquisition.py` (Native evidence and full-session transcript lifecycle acquisition)
- IMPLEMENTS → CODE_FILE `src/aptl/backends/scenario_capture.py` (Request-scoped installed capture contribution contract)
- IMPLEMENTS → CODE_FILE `src/aptl/backends/scenario_capture_discovery.py` (Exact pack/backend capture-provider selection and validation)
- IMPLEMENTS → CODE_FILE `src/aptl_techvault/capture.py` (TechVault capture declarations and bounded runtime operations)
- IMPLEMENTS → CODE_FILE `src/aptl_techvault/evidence/techvault.py` (Bounded TechVault source validation and evidence shaping)
- IMPLEMENTS → CODE_FILE `src/aptl_techvault/evidence/techvault_native.py` (Backend-native Cortex, Suricata, and Wazuh acquisition)
- IMPLEMENTS → CODE_FILE `src/aptl/core/deployment/_compose_capture_apparatus.py` (Scope-admitted capture realization, activation, observation, and export)
- IMPLEMENTS → CODE_FILE `src/aptl/backends/raes_runtime_attestation.py` (Exact-release configuration corroboration)
- IMPLEMENTS → DOCUMENTATION `docs/architecture/issue-992-backend-observability-ownership-preflight.md` (Evidence ownership, intrusion, disclosure, and lifecycle contract)
- TESTS → TEST `tests/test_backend_observability.py` (Open/closed scope and minimum-intrusion apparatus selection)
- TESTS → TEST `tests/test_capture_apparatus.py` (Capture realization disclosure and fail-closed lifecycle controls)
- TESTS → TEST `tests/test_raes_evidence.py` (Canonical demand admission and unsupported-evidence rejection)
- TESTS → TEST `tests/test_raes_evidence_acquisition.py` (Native and transcript acquisition, tamper rejection, and terminal outcomes)
- TESTS → TEST `tests/test_raes_native_evidence_truth.py` (Exact evidence-to-proposition binding and cardinality)
- TESTS → TEST `tests/test_raes_runtime_attestation.py` (Exact-release attestation and drift rejection)
- TESTS → TEST `tests/test_techvault_evidence_sources.py` (TechVault native source validity and failure cases)
- TESTS → TEST `tests/test_techvault_native_evidence.py` (End-to-end native evidence owner contract)
- TESTS → TEST `tests/test_pack_adapter_install_seam.py` (Request-scoped capture registry and generic adapter-bound collector wiring)
