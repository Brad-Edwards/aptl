...
- Path-traversal risk in `core.credentials.sync_dashboard_config` / `sync_manager_config` (Sonar BLOCKER S6781 — `new_security_rating=5` on `dev`). Both functions now take `project_dir` plus build the canonical project-relative path internally (`config/wazuh_dashboard/wazuh.yml` and `config/wazuh_cluster/wazuh_manager.conf`) and validate containment via `Path.resolve()` + `is_relative_to` before any I/O. Symlinks under the canonical location pointing outside the project root are rejected with `ValueError`. Implements [#266](https://github.com/Brad-Edwards/aptl/issues/266); architectural guardrail recorded in [ADR-007 § Security Guardrail](docs/adrs/adr-007-python-cli-control-plane.md). `_step_sync_credentials` in `core/lab.py` simplified to pass only `ctx.project_dir`. New `TestPathContainment` class in `tests/test_credentials.py` covers symlink-escape rejection and write-to-canonical-only behavior; existing tests pivoted from the per-test file path to the new (project_dir, secret) shape.
