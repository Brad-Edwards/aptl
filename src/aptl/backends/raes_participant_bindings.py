...
class _BindingContext:
    ...
    def _compose_service(self, node: NodeRealization) -> ComposeServiceInfo | None:
        """Return the Compose service bound to a realized RAES node."""
...
def _select_service_address(
    ...
    if source_node is not None:
        ...
        value = _address_for_raes_networks(service, shared)
        if value is not None:
            return value
    value = _address_for_raes_networks(service, set(target_node.networks))
    if value is not None:
        return value
    return next(
...
def _address_for_raes_networks(
    service: ComposeServiceInfo,
    raes_networks: set[str],
) -> str | None:
    """Return a static service address on one of the requested RAES networks."""

    desired_aliases = set().union(
        *(_network_aliases(network) for network in raes_networks)
    )
...
def _network_aliases(raw: str) -> set[str]:
    """Return comparable aliases for RAES and Compose network identifiers."""
...
def _node_service_ref(ref: str) -> tuple[str, str] | None:
    """Split a RAES node service ref into node and service names."""
...
def _node_name(ref: str) -> str | None:
    """Return a node name from RAES shorthand or compiled node refs."""
