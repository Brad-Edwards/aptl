...
- The proof must enter through the public startup path: catalog resolution via
  ...
  `_step_start_containers()` / `start_raes_scenario()`. A direct
  `AptlProvisioner.apply()` call is acceptable only for static expectations,
  never as live boot proof.
- The expected live surface is model-derived and reduced. Compute it from RAES
...
- Passing proof means the post-boot containers and networks match the
  RAES-realized selected Compose profile set. Because Compose activates every
  service in a selected profile, expected containers are the steady-state
  services selected by those profiles, not only the declared RAES nodes.
...
- Scenario authority: `scenarios/catalog.json`, `aptl.core.scenario_catalog`,
  `raes.parse_sdl_file`, RAES import/compile/planning diagnostics, and
  `RuntimeManager.plan()`.
- Realization and profile authority: `aptl.backends.raes_realization`,
  `aptl.backends.raes_dependency_closure`, `aptl.backends.raes_profiles`,
...
- Startup authority: `orchestrate_lab_start()`, `_LAB_START_STEPS`,
  `_LabStartContext.selected_profiles`, `start_raes_scenario()`, `stop_lab()`,
...
- **Catalog and path containment:** selected variants must pass
  ...
  and RAES parser validation. Do not add a scenario-name switch or path
  allowlist outside the catalog/explicit-path resolver.
- **RAES shape and backend contracts:** parse, compile, plan, realization,
  backend manifest, and profile conformance stay RAES-owned. APTL proof code
  must not create a local Pydantic mirror of RAES SDL or flatten away RAES
...
realization_details)` plus a model-derived expected live matrix keyed by RAES
...
`load_compose_profile_index()` exposes today, extend `aptl.backends.raes_profiles`
...
- Scraping human `docker compose` errors, CLI text, or logs to decide pass/fail
  when RAES diagnostics, `LabResult`, `StartupDiagnostic`, `RangeSnapshot`, and
...
- Do not change the default public scenario, full TechVault live gate, static
  gate, parity inventory, RAES backend profile claim, or Docker Compose profile
  topology.
