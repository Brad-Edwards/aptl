...
# Capcom CTF Victim module - conditional
module "capcom_victim" {
  count  = var.enable_capcom_victim ? 1 : 0
  source = "./modules/capcom-victim"

  subnet_id                   = module.network.subnet_id
  security_group_id           = module.network.victim_security_group_id
  capcom_victim_ami           = var.capcom_victim_ami
  capcom_victim_instance_type = var.capcom_victim_instance_type
  key_name                    = var.key_name
  siem_private_ip             = local.siem_private_ip
  siem_type                   = var.siem_type
  project_name                = var.project_name
