...
- `ParticipantRuntime` is a fourth RAES backend component on the existing
...
- The backend manifest promotion is all-or-nothing: `create_aptl_manifest()`
  ...
  published `raes conformance backend` CLI must pass the promoted profile with
  no `conformance.unsupported-capability-claim` diagnostics.
- APTL consumes the published RAES participant contracts as-is. Episode state
...
- The control-plane path under proof is the RAES control plane:
...
- RAES authorities: `raes_backend_protocols.protocols.ParticipantRuntime`,
...
- Existing APTL adapter seams: `create_aptl_runtime_target()`,
  ...
  `AptlRealization.details()`, and `render_raes_diagnostics()`.
...
- Live proof owners: `aptl.validation.curated_live_proof`,
  ...
  `docs/raes/techvault-curated-live-validation-gate/`, and the full live-gate
...
- Repo gates: `pytest`, `pre-commit run --all-files`, manual
  `raes-scenario-gate`, live-gate integration marker, `.gc/plan-rules.md`, and
  Conventional Commit PR titles (release-please drives releases).

## Security And Validation Layers

- **RAES component shape:** `RuntimeTarget` validates manifest/component
...
- **Manifest and conformance:** `ParticipantRuntimeCapabilities` validates
  ...
  `run_target_conformance()` and `raes conformance backend` reject overclaims.
- **Episode and behavior envelopes:** participant state/history must be created
  through the RAES dataclasses or validated with the RAES snapshot violation
...
- **Container/project isolation:** action target resolution must be derived from
  RAES realization details and the project-scoped backend/snapshot. Do not
...
- **Error envelopes:** RAES-facing failures are `Diagnostic` records and
...
- Do not add new RAES schemas, profiles, or controlled vocabulary terms.
